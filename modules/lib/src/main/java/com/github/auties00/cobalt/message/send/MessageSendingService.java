 ...
 *
 * <p>The service is the single entry point for the send pipeline. It
 * prepares a raw {@link MessageContainer} into a fully-populated
 * {@link MessageInfo} via {@link MessagePreparer}, dedupes concurrent sends
 * keyed by message id via {@link MessageDedup}, brackets the dispatch in a
 * {@code WebcMessageSend} WAM event, then routes by the parent JID's
 * server kind to one of the per-chat-kind sub-senders:
 *
 * <ul>
 *   <li>1:1 PN or LID chat to {@link UserMessageSender}: per-device Signal
 *       fanout with optional phash-mismatch resend.</li>
 *   <li>group or community to {@link GroupMessageSender}: sender-key
 *       SKMSG, sender-key distribution on first contact, addressing-mode
 *       migration on stale-ack errors.</li>
 ...
 *   <li>business broadcast list to {@link BroadcastMessageSender}: SKMSG
 *       against the audience read from the local
 *       {@link com.github.auties00.cobalt.model.business.BusinessBroadcastList}
 *       roster.</li>
 *   <li>newsletter to {@link NewsletterMessageSender}: plaintext SMAX
 *       publish, no Signal envelope.</li>
 * </ul>
 *
...
public final class MessageSendingService {
    /**
     * The {@link MessagePreparer} that turns a raw {@link MessageContainer}
     * into a populated {@link MessageInfo}.
     */
    private final MessagePreparer preparer;

    /**
     * The {@link MessageDedup} tracker that rejects a second concurrent send
     * with the same wire id.
     */
    private final MessageDedup messageDedup;

    /**
     * The {@link UserMessageSender} for 1:1 PN- or LID-addressed chats.
     */
    private final UserMessageSender userSender;

    /**
     * The {@link GroupMessageSender} for group and community chats.
     */
    private final GroupMessageSender groupSender;

    /**
     * The {@link StatusMessageSender} for status broadcasts.
     */
    private final StatusMessageSender statusSender;

    /**
     * The {@link BroadcastMessageSender} for business broadcast-list
     * sends addressed via {@code <id>@broadcast}.
     */
    private final BroadcastMessageSender broadcastSender;

    /**
     * The {@link NewsletterMessageSender} for plaintext SMAX newsletter
     * publishes.
     */
    private final NewsletterMessageSender newsletterSender;

    /**
     * The {@link PeerMessageSender} for app-state-sync and other
     * own-device peer protocol stanzas.
     */
    private final PeerMessageSender peerSender;

    /**
     * The {@link WhatsAppClient} used by the orchestrator to reach shared
     * services not already injected into sub-senders.
     */
    private final WhatsAppClient client;

    /**
     * The {@link WamService} used to record per-send {@code WebcMessageSend}
     * events.
     */
    private final WamService wamService;

    /**
     * The {@link MediaTranscoderService} consulted to attach a rich
     * preview to outgoing extended-text messages whose body contains a
     * URL.
     */
    private final MediaTranscoderService mediaTranscoderService;

    /**
     * Constructs a {@link MessageSendingService} and wires up every
     * sub-sender.
     *
     * @apiNote
     * Embedders construct exactly one of these per client; the constructor
     * also instantiates the link-preview pipeline, the sender-key
     * distribution helper, and the four stanza-decoration builders
     * ({@link BotStanza}, {@link BizStanza}, {@link MetaStanza},
     * {@link ReportingStanza}) that the sub-senders share.
     *
     ...
     *                            by the user-chat sender to gate the
     *                            PN-to-LID stanza rewrite
     * @param abPropsService      the {@link ABPropsService} consulted for
     *                            feature-gating decisions
     * @param wamService             the {@link WamService} that records
     *                               per-send events
     * @param mediaTranscoderService the {@link MediaTranscoderService}
     *                               consulted for link-preview
     *                               decoration on outgoing text
     *                               messages
     ...
     *
     * @apiNote
     * Top-level entry point for embedders sending a message: the call
     ...
     * encrypted addon variants in CAG groups, decorates an extended-text
     * body with a link preview when applicable, and then forwards to the
     ...
     *
     * @apiNote
     * Use this overload when the caller has already prepared the
     * {@link MessageInfo} (typically a resend, a debug-injection harness, or
     * a follow-up to a {@code sendKeyDistribution} call). The routing
     * predicate is keyed on the message-info subtype combined with the
     ...
     *                                                         is missing the
     *                                                         id or parent JID
     * @throws WhatsAppMessageException.Send.InvalidRecipient  if the parent
     *                                                         JID's server
     *                                                         does not match
     *                                                         the
     *                                                         {@link MessageInfo}
     ...
     *
     * @apiNote
     * Suppressed for newsletter sends (a different WA Web logger covers
     * those) and for protocol revoke messages. For every other chat message
     * the builder is pre-populated with the resolved {@code messageType},
     * {@code messageMediaType}, the {@code messageIsForward} flag taken from
     * the optional {@link ContextualMessage#contextInfo()}, and the
     * latency timer is started so it can be stopped before the
     * success-branch commit on the caller side.
     *
     ...
     *
     * @apiNote
     * Use to pre-seed sender keys before sending a media-heavy or otherwise
     * latency-sensitive message; the call returns silently when every
     * participant already holds the key. The stanza carries the
     * {@code text} type marker and {@code device_fanout="false"}.
     *
     * @param groupJid the target group {@link Jid}
     * @param key      the {@link MessageKey} carrying the wire id and parent
     *                 JID
     * @throws NullPointerException                            if any argument
     *                                                         is {@code null}
     * @throws IllegalArgumentException                        if the key
     *                                                         has no id
     * @throws WhatsAppMessageException.Send.InvalidRecipient  if
     *                                                         {@code groupJid}
     *                                                         is not a group
     *                                                         or community
     */
     ...
     *
     * @apiNote
     * Peer messages cover app-state sync, key shares, fatal-exception
     * notifications, and peer-data operation requests and responses; the
     * wire stanza is encrypted per device and tagged with
     * {@code category="peer"} so the server routes it to the linked-device
     * shelf rather than to the main chat fanout.
     *
     * @param targetDevice the target device {@link Jid}, typically the
