...
/**
 * Parses the Facebook GraphQL response of the Australian financial-services regulated-category query built by
 * {@link CometRegulatedCategoryUtilsAustraliaFinservFacebookGraphQlRequest} into an
 * {@link AdTargetingComplianceStatus}.
 *
 * <p>Reads the linked {@code lwi} root and projects its scalar
 * {@code target_spec_subject_to_australia_finserv} onto the shared
 * {@link AdTargetingComplianceStatus} verdict.
 *
 * @see CometRegulatedCategoryUtilsAustraliaFinservFacebookGraphQlRequest
 */
@WhatsAppWebModule(moduleName = "LWICometRegulatedCategoryUtilsAustraliaFinservQuery")
public final class CometRegulatedCategoryUtilsAustraliaFinservFacebookGraphQlResponse implements FacebookGraphQlOperation.Response {
    /**
     ...
     */
    private CometRegulatedCategoryUtilsAustraliaFinservFacebookGraphQlResponse(AdTargetingComplianceStatus status) {
        this.status = status;
    }
     ...
     */
    public static Optional<CometRegulatedCategoryUtilsAustraliaFinservFacebookGraphQlResponse> of(JSONObject data) {
        if (data == null) {
        ...
        var subject = root.getBoolean("target_spec_subject_to_australia_finserv");
        var status = new AdTargetingComplianceStatusBuilder()
                .subject(subject != null && subject)
                .build();
        return Optional.of(new CometRegulatedCategoryUtilsAustraliaFinservFacebookGraphQlResponse(status));
    }
