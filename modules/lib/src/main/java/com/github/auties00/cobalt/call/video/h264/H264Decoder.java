...
public final class H264Decoder implements AutoCloseable {
    ...
    private Frame readFrame() {
        ...
        int w = TagSysMemBuffer.iWidth(sysBuf);
        int h = TagSysMemBuffer.iHeight(sysBuf);
        int yStride = TagSysMemBuffer.iStride(sysBuf, 0);
        int uvStride = TagSysMemBuffer.iStride(sysBuf, 1);
        int ySize = w * h;
        int uvSize = (w / 2) * (h / 2);
