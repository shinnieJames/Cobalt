...
/**
 * Bridges the raw-I420 video ports of an {@link CallRuntime} and the encoded {@link VideoPacket} ports
 ...
 * {@link CallRuntime#takeOutboundVideo()}, forces a keyframe when {@link #requestKeyframe()} has been
 ...
 * {@link CallRuntime#deliverInboundVideo(VideoFrame)} for rendering. The BWE driver reconfigures the
 * encoder at runtime through {@link #adjustBitrate(int)}.
 *
 * <p>The pipeline owns two virtual threads: a capture thread that drains
 * {@link CallRuntime#takeOutboundVideo()} and encodes, and a decoder thread that drains the inbound
...
public final class VideoPipeline implements AutoCloseable {
    ...
    /**
     * Holds the capture thread that drains {@link CallRuntime#takeOutboundVideo()} and encodes;
     ...
     *
     * @param call         the {@link CallRuntime} to drive
     ...
     */
    public VideoPipeline(CallRuntime call, Consumer<VideoPacket> outboundSink,
     ...
     *
     * @implNote This implementation paces outbound frames to wall-clock against a monotonic anchor taken
     * with the first frame's presentation timestamp: each frame is held until its timestamp, relative to
     * that first frame, is due. A source that yields frames faster than real time (a file) is therefore
     * transmitted at its natural rate rather than flushed to the peer in a burst, while a self-paced
     * source (a camera or screen-grab device) keeps its timestamps at or behind the anchor and is never
     * delayed.
     */
    private void captureLoop() {
        ...
        while (running) {
            ...
            if (anchorNanos < 0) {
                ...
                firstPtsMs = frame.ptsMs();
            } else if (!sleepUntil(anchorNanos + (frame.ptsMs() - firstPtsMs) * 1_000_000L)) {
                return;
            }
    ...
    /**
     * Parks the capture thread until the given monotonic deadline, pacing outbound video to wall-clock.
     *
     ...
     * delivers any resulting {@link VideoFrame} to {@link CallRuntime#deliverInboundVideo(VideoFrame)}.
