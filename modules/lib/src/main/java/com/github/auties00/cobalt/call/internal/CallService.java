...
import com.github.auties00.cobalt.call.internal.signaling.*;
...
public class CallService {
    ...
    private static CallResultType mapToResultType(CallEndReason reason) {
        return switch (reason) {
            ...
            // accepted_elsewhere lands as a peer-side terminate; from the
            // local user's perspective the call did connect, just not on
            // this device.
    ...
    /**
     * Sends an in-call interaction over the pre-negotiated
     * DataChannel of the call identified by {@code callId}.
     *
     * @apiNote Routes through the encrypted DataChannel path
     * decoded in Phase 6 (see {@code reference_wa_voip_interaction_wire_format}
     * memory): the interaction is encoded into an RTP/RTCP-shaped
     * packet by {@link CallInteractionEncoder}, encrypted via the
     * call's negotiated SRTP keys, then handed to the call's
     * default DataChannel. No-op if no call is registered for
     * {@code callId}, if the transport is not open, or if the SRTP
     * endpoint is not yet available.
     * @implNote The plaintext body of each interaction is
     * empirically guessed (the live WA wasm encrypts before
     * publishing, so the exact plaintext is not recoverable from
     * captures alone). End-to-end validation against a real WA
     * peer is deferred to Phase 8. The {@code peer} and
     * {@code creator} parameters are kept for ABI compatibility
     * with the previous stanza-based stub; they are unused in the
     * DataChannel path.
     *
     * @param peer        the peer JID (unused in this path)
     * @param creator     the call creator JID (unused in this path)
     * @param callId      the call identifier
    ...
    public void sendInteraction(Jid peer, Jid creator, String callId, CallInteraction interaction) {
        ...
        // byte0's top nibble distinguishes RTP (0x80/0x81) from
        // RTP-with-marker (0x90/0x91). Both still ride the RTP SRTP
        // path here; SRTCP would be triggered by RTCP packet types
        // (200-204) at byte 1, but the WAVoip interaction stream
        // uses RTP framing even where byte 1 is 0xc8 — that 0xc8
        // is a payload-type literal, not an RTCP packet type. So we
        // always use protectRtp for these envelopes.
