...
public final class DtlsSrtpEndpoint {
     ...
     * @return a primed {@link SrtpEndpoint}
     */
    private SrtpEndpoint handshakeClient(JcaTlsCrypto crypto) throws IOException {
        var client = new CobaltDtlsClient(crypto, localCert, expectedPeerFingerprint);
        new DTLSClientProtocol().connect(client, transport);
        return SrtpEndpoint.fromDtlsKeyingMaterial(client.exportedKeyingMaterial(), role);
    }
     ...
     * @return a primed {@link SrtpEndpoint}
     */
    private SrtpEndpoint handshakeServer(JcaTlsCrypto crypto) throws IOException {
        var server = new CobaltDtlsServer(crypto, localCert, expectedPeerFingerprint);
        new DTLSServerProtocol().accept(server, transport);
        return SrtpEndpoint.fromDtlsKeyingMaterial(server.exportedKeyingMaterial(), role);
    }
