...
/**
 * Parses the Facebook GraphQL response of the India financial-services regulated-category query built by
 * {@link CometRegulatedCategoryUtilsIndiaFinservFacebookGraphQlRequest} into an
 * {@link AdTargetingComplianceStatus}.
 *
 * <p>Reads the linked {@code lwi} root and projects its scalar
 * {@code target_spec_subject_to_india_finserv} onto the shared
 * {@link AdTargetingComplianceStatus} verdict.
 *
 * @see CometRegulatedCategoryUtilsIndiaFinservFacebookGraphQlRequest
 */
@WhatsAppWebModule(moduleName = "LWICometRegulatedCategoryUtilsIndiaFinservQuery")
public final class CometRegulatedCategoryUtilsIndiaFinservFacebookGraphQlResponse implements FacebookGraphQlOperation.Response {
    /**
     ...
     */
    private CometRegulatedCategoryUtilsIndiaFinservFacebookGraphQlResponse(AdTargetingComplianceStatus status) {
        this.status = status;
    }
     ...
     */
    public static Optional<CometRegulatedCategoryUtilsIndiaFinservFacebookGraphQlResponse> of(JSONObject data) {
        if (data == null) {
        ...
        var subject = root.getBoolean("target_spec_subject_to_india_finserv");
        var status = new AdTargetingComplianceStatusBuilder()
                .subject(subject != null && subject)
                .build();
        return Optional.of(new CometRegulatedCategoryUtilsIndiaFinservFacebookGraphQlResponse(status));
    }
