...
public final class DtlsSrtpDriver implements AutoCloseable {
     ...
     * endpoint + DTLS application-data transport, or the failure
     * that ended the handshake.
     */
    private record HandshakeResult(SrtpEndpoint srtp, DTLSTransport dtls, IOException failure) {
         ...
         * @param dtls the BC application-data DTLS transport
    ...
    /**
     * Returns the BouncyCastle {@link DTLSTransport}
     * for sending and receiving application-data records over the
     * negotiated DTLS session, or {@code null} if the handshake has
     * not completed yet.
     *
     * <p>The data-channel layer (#60) sends SCTP packets through this
     * transport — {@code send(buf, off, len)} writes one encrypted
     * record; {@code receive(buf, off, len, waitMillis)} reads the
     * next decrypted one.
     *
     * @return the BC DTLS transport, or {@code null}
     */
