 ...
 *
 * <p>This is the device-backed {@link AudioInputStream} returned by {@link AudioInputStream#toSpeaker()}.
 * Its constructor acquires a {@link SourceDataLine} for signed 16-bit mono PCM, defaulting to the 16 kHz
 * wire rate of a Cobalt call, opens it, and starts it. Each {@link #offer(AudioFrame)} converts the
 * frame's samples to little-endian bytes and pushes them to the line, blocking while the line's buffer
 * is full so playback paces the call. {@link #shutdown()} drains and releases the line. The stream is
 * mono only; the operating system spreads the single channel across whatever channels the physical
 * speaker has.
 *
 ...
 */
public final class SpeakerAudioInputStream extends AudioInputStream {
    /**
     ...
     */
    public SpeakerAudioInputStream() {
     ...
     * one was supplied or the JVM default device otherwise, opens it, and starts it so offered frames
     * play immediately.
     *
     ...
     */
    public SpeakerAudioInputStream(int sampleRate, Mixer.Info preferredMixer) {
        if (sampleRate < 1) {
     ...
     * {@link #shutdown()} is dropped, and any line write failure is swallowed so a transient device
     * fault does not stall the call.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>Marks the stream ended, then drains so buffered audio finishes playing, stops the line, and
     * closes it. Each step ignores failures so a fault in one does not leak the line, making the call
     * idempotent.
     */
    @Override
