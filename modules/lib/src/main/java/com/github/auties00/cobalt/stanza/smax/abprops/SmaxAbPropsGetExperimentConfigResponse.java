 ...
 * bundle into the local AB-prop store or back off, and {@link #of(Stanza, Stanza)} disambiguates the
 * three by trying each variant in turn.
 */
public sealed interface SmaxAbPropsGetExperimentConfigResponse extends SmaxStanza.Response
    ...
    static Optional<? extends SmaxAbPropsGetExperimentConfigResponse> of(Stanza stanza, Stanza request) {
     ...
     * and schedule the next sync; the raw {@code <props/>} stanza is retained for the caller to re-parse
