...
public final class AudioPipeline implements AutoCloseable {
    ...
    private void captureLoop() {
        long ptsMs = 0;
        long frameDurMs = 1000L * options.frameSize() / options.sampleRate();
        while (running) {
            AudioFrame frame;
            try {
            ...
            short[] processed = processCaptureFrame(frame.pcm());
            boolean voiceActive = processed != null && runPreprocessor(processed);
            if (processed == null) {
                continue;
            }
            byte[] payload;
