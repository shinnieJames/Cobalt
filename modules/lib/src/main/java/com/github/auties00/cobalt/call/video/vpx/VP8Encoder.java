...
import com.github.auties00.cobalt.call.video.vpx.bindings.vpx_codec_cx_pkt;
...
public final class VP8Encoder implements AutoCloseable {
    ...
    public List<Packet> encode(byte[] yuvI420, long pts, boolean forceKeyFrame) {
        ...
        loadImage(yuvI420);
        int flags = forceKeyFrame ? LibVpx.VPX_EFLAG_FORCE_KF() : 0;
        int rc;
        try {
    ...
    private List<Packet> drainPackets() {
        ...
        while (true) {
            ...
            int kind = vpx_codec_cx_pkt.kind(pkt);
            ...
            long sz = vpx_codec_cx_pkt.data.frame.sz(frame);
            long pts = vpx_codec_cx_pkt.data.frame.pts(frame);
            int flags = vpx_codec_cx_pkt.data.frame.flags(frame);
