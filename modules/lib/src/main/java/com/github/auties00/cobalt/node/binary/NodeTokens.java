...
/**
 * Holds the static token dictionaries used to compress repeated strings in
 * WhatsApp's binary stanza protocol.
 *
 * <p>Every tag name, attribute key, attribute value, or text payload that
 * appears in a stanza is first looked up in these tables. A hit yields a
 * one byte token (single byte dictionary) or a two byte token (dictionary
 * index) in place of the raw UTF-8 bytes. The dictionaries are versioned
 * alongside the protocol and the current revision is exposed via
 * {@link #DICTIONARY_VERSION}.
 *
 * <p>Each public field is a fully built dictionary that exposes
...
public final class NodeTokens {
    /**
     * Current revision of the WhatsApp binary dictionary tables.
     */
    ...
    /**
     * Dictionary of strings reachable through a two byte wire token whose
     * leading byte is {@link NodeTags#DICTIONARY_0}.
     */
    ...
    /**
     * Dictionary of strings reachable through a two byte wire token whose
     * leading byte is {@link NodeTags#DICTIONARY_1}.
     */
    ...
    /**
     * Dictionary of strings reachable through a two byte wire token whose
     * leading byte is {@link NodeTags#DICTIONARY_2}.
     */
    ...
    /**
     * Dictionary of strings reachable through a two byte wire token whose
     * leading byte is {@link NodeTags#DICTIONARY_3}.
     */
    ...
    /**
     * Aggregate array exposing the four two byte dictionaries in the order
     * implied by their {@link NodeTags#DICTIONARY_0} to {@link NodeTags#DICTIONARY_3}
     * leading tags.
     */
    ...
    /**
     * Forward table mapping a token index to its string value.
     */
    private final String[] values;

    /**
     * Reverse table mapping a string value to its token index for constant
     * time lookups in {@link #indexOf(String)}.
     */
     ...
     *
     * @param values the string values to index
     */
    ...
    /**
     * Returns the token index of the supplied string in this dictionary.
     *
     * @param value the string to look up
     * @return the matching index or {@code -1} when the value is absent
     */
    ...
    /**
     * Returns the string stored at the supplied token index in this
     * dictionary.
     *
     * @param index the token index to look up
     * @return the matching string or {@code null} when the index is out
     *         of range
     */
