...
/**
 * Holds the static token dictionaries used to compress repeated strings
 * in WhatsApp's binary protocol.
 *
 * <p>Every string appearing in a stanza (tag name, attribute key or
 * value, content) is first looked up in these tables: a hit yields a
 * single-byte token (or a two-byte dictionary-index token) instead of
 * the raw UTF-8 bytes. The dictionaries are versioned alongside the
 * WhatsApp protocol; the current version is reported by
 * {@link #DICTIONARY_VERSION}.
 *
 * <p>The class doubles as a container for a single dictionary instance:
 * each public field holds a {@code NodeTokens} that exposes
 * {@link #indexOf(String)} and {@link #get(int)} for use by the encoder
 * and decoder.
 *
...
public final class NodeTokens {
    /**
     * The current version of the WhatsApp binary dictionary.
     *
     * @implNote WAWapDict.DICT_VERSION
     */
    ...
    /**
     * First dictionary of two-byte tokens, prefixed by
     * {@link NodeTags#DICTIONARY_0}.
     *
     * @implNote WAWapDict.DICTIONARY_0_TOKEN
     */
    ...
    /**
     * Second dictionary of two-byte tokens, prefixed by
     * {@link NodeTags#DICTIONARY_1}.
     *
     * @implNote WAWapDict.DICTIONARY_1_TOKEN
     */
    ...
    /**
     * Third dictionary of two-byte tokens, prefixed by
     * {@link NodeTags#DICTIONARY_2}.
     *
     * @implNote WAWapDict.DICTIONARY_2_TOKEN
     */
    ...
    /**
     * Fourth dictionary of two-byte tokens, prefixed by
     * {@link NodeTags#DICTIONARY_3}.
     *
     * @implNote WAWapDict.DICTIONARY_3_TOKEN
     */
    ...
    /**
     * Reverse lookup from string to index, used by
     * {@link #indexOf(String)} in constant time.
     */
    private final Map<String, Integer> map;

    /**
     * Builds a dictionary from the supplied strings in the order they
     * appear.
     *
     * @param values the string values, in positional order
     */
    ...
    /**
     * Returns the token index of the given string, or {@code -1} if the
     * string is not in this dictionary.
     *
     * @param value the string value to look up
     * @return the index associated with the value, or {@code -1} if not
     *         found
     */
    ...
    /**
     * Returns the string at the given token index, or {@code null} if the
     * index is out of range.
     *
     * @param index the token index to look up
     * @return the string at {@code index}, or {@code null} if out of bounds
     */
