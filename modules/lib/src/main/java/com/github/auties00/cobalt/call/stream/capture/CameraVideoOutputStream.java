 ...
 * {@link VideoOutputStream#fromCamera()}. Its constructor opens a libavdevice capture device, demuxes
 * its video stream, decodes it, and arranges to convert each decoded picture to I420 with libswscale so
 ...
 * device's native capture resolution. The four-argument geometry constructor auto-detects the running
 * platform and opens its conventional default camera; {@link #CameraVideoOutputStream(String, int, int, int, int)}
 * opens a named device using the platform-default input format; and
 * {@link #CameraVideoOutputStream(String, String, int, int, int, int)} is the power-user entry point
 * that opens an explicit libavdevice input-format and URL pair for cases the platform default does not
 * cover. The same pipeline backs the screen-grab streams created through {@link ScreenVideoOutputStream}.
 *
 ...
 *       {@link #CameraVideoOutputStream(String, int, int, int, int)} must be given the device's friendly
 *       name (for example {@code "Integrated Camera"}), because libavdevice's {@code dshow} input format
 *       has no stable default index</li>
 * </ul>
 *
 ...
 */
public sealed class CameraVideoOutputStream extends VideoOutputStream
        permits ScreenVideoOutputStream {
    /**
     ...
     *
     * <p>Equivalent to {@link #CameraVideoOutputStream(String, String, int, int, int, int)} with the
     ...
     */
    public CameraVideoOutputStream(int width, int height, int fps, int bitrateBps) {
     ...
     * @throws IllegalStateException    if the device cannot be opened or has no video stream
     */
    public CameraVideoOutputStream(String deviceUrl, int width, int height, int fps, int bitrateBps) {
     ...
     * {@link #CameraVideoOutputStream(int, int, int, int)} nor
     * {@link #CameraVideoOutputStream(String, int, int, int, int)} selects the right device. If any step
     ...
     */
    public CameraVideoOutputStream(String indev, String url, int width, int height, int fps, int bitrateBps) {
     ...
     * {@inheritDoc}
     *
     * <p>Reads packets from the device until one decodes into a frame, converts that frame to I420, and
     * returns it. Packets belonging to other streams and decoder "need more input" results are skipped
     * transparently. Returns {@code null} when the device reports an unrecoverable end-of-input or once
     ...
     * is available, which paces a live camera to its native frame rate, exactly as the old capture pump
     * was paced by the same blocking read.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>Marks the stream ended and frees the libswscale converter and every libav* allocation, then
        ...
        if (os.contains("win")) {
            throw new UnsupportedOperationException(
                    "Windows has no default camera URL - pass a device "
                            + "name to CameraVideoOutputStream(String), e.g. \"Integrated Camera\"");
        }
        return "/dev/video0";
    }
    ...
    /**
     * Converts the current decoded frame to I420 with libswscale and wraps it in a {@link VideoFrame}.
     *
     * <p>Rejects frames whose dimensions are below {@code 2} or odd, since I420's half-resolution chroma
     * planes require even dimensions. Rebuilds the libswscale converter when the frame's dimensions or
     * source pixel format differ from the converter's current configuration, scales the three planes
     * into a contiguous I420 buffer, and stamps the frame with the current wall clock.
     *
