 ...
 *
 * @apiNote
 * Implementations are the application-level glue between the transport
 ...
 * {@link WhatsAppSocketClient#connect(WhatsAppSocketListener)} call;
 * after {@link #onClose()} fires no further callbacks are delivered and
 * a fresh listener (or the same one, reset) must be supplied on the
 * next {@code connect}.
 */
public interface WhatsAppSocketListener {
     ...
     *
     * @apiNote
     * Invoked once per inbound int24-framed AES-GCM datagram. The call
     * runs on the socket reader virtual thread and must not block on
     * I/O of the same {@link WhatsAppSocketClient}: doing so would
     * deadlock the reader and stall every subsequent inbound node on
     * the same socket.
     *
     * @implSpec
     ...
     *
     * @apiNote
     * Errors surface here rather than being thrown at the application
     * thread because the reader runs asynchronously on a virtual
     * thread. The exception type discriminates recovery action: see
     ...
     * {@link WhatsAppSocketClient#connect(WhatsAppSocketListener)}
     * regardless of whether the close was orderly (caller invoked
     * {@link WhatsAppSocketClient#disconnect()}) or remote-initiated
     * (server dropped the connection or the reader hit a fatal error).
     * No further {@link #onNode(Node)} or {@link #onError(WhatsAppException)}
     * callbacks will be delivered after this call returns.
     */
    void onClose();
}
