...
/**
 * Holds the static token dictionaries that compress repeated strings in
 * WhatsApp's binary stanza protocol.
 *
 * <p>Every tag name, attribute key, attribute value, or text payload that
 * {@link StanzaWriter} emits is first looked up in these tables. A hit in
 * {@link #SINGLE_BYTE_TOKENS} produces a one-byte token; a hit in one of the
 * four extension dictionaries produces a two-byte token (dictionary tag plus
 * index). A miss falls through to a length-prefixed UTF-8 encoding. The same
 * tables drive {@link StanzaReader} in reverse. The dictionary contents are
 * versioned with the protocol; {@link #DICTIONARY_VERSION} exposes the current
 * revision so a peer that publishes a newer table can be detected.
 *
 * @implNote
 * This implementation indexes each table eagerly at construction time into an
 * unmodifiable {@link HashMap} so the encode-side {@link #indexOf(String)}
 * lookup is O(1); the cost is amortised across every stanza the long-lived
 * encoder ever sends.
 *
 * @see Stanza
 * @see StanzaWriter
...
public final class StanzaTokens {
    ...
    /**
     * Holds the forward table mapping a token index to its string value,
     * indexed by the wire token byte and consumed by {@link #get(int)} during
     * decode.
     */
    private final String[] values;

    /**
     * Holds the reverse table mapping a string value to its token index for
     * the constant-time {@link #indexOf(String)} lookup during encode.
     *
     * <p>Built once at construction and never modified.
     */
    private final Map<String, Integer> map;

    /**
     * Builds a dictionary from the supplied strings in positional order.
     *
     * <p>The reverse map is wrapped {@link Collections#unmodifiableMap
     * unmodifiable} so accidental mutation is impossible.
     *
     * @param values the string values to index, in wire-token order
     */
    private StanzaTokens(String... values) {
        this.values = values;
        Map<String, Integer> map = HashMap.newHashMap(values.length);
    ...
    /**
     * Returns the wire token index of the supplied string in this dictionary,
     * or {@code -1} when the string is not present.
     *
     * <p>A {@code -1} return lets {@link StanzaWriter} and {@link StanzaSizer}
     * fall through to the next compression strategy.
     *
     * @param value the string to look up
     * @return the matching index in the range {@code 0..255}, or {@code -1}
    ...
    /**
     * Returns the string stored at the supplied wire token index in this
     * dictionary, or {@code null} when the index is out of range.
     *
     * @param index the wire token index to look up
     * @return the matching string, or {@code null} when {@code index} is
     *         negative or greater than or equal to the dictionary size
     */
