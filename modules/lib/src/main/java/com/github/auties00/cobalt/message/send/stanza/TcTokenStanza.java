 ...
 * lower-priority {@code <cstoken>} fallback in {@link CsTokenStanza}.
 *
 * <p>This class also hosts the token-lifetime helpers: {@link #getTcTokenDuration(TcTokenMode)},
 * {@link #tokenExpirationCutoff(TcTokenMode)}, {@link #isTokenExpired(Instant, TcTokenMode)},
 * {@link #shouldSendNewToken(Instant)}, {@link #encodeTcTokenForMex(byte[])} and {@link TcTokenMode}.
 *
 * @implNote
 * This implementation co-locates the lifetime helpers from {@code WAWebTrustedContactsUtils} because
 * that JS module is stateless (it reads AB props and the system clock only) and its single Cobalt
 * caller lives in this class. The helpers and {@link TcTokenMode} are kept package-private even
 * though they are not called externally, so the relationship to the WA Web exports stays expressible
 * through the source-provenance annotations.
 */
...
public final class TcTokenStanza {
     ...
     *
     * @implNote
     * This implementation queries the receiver-side expiry parameters ({@link TcTokenMode#RECEIVER});
     * the sender-side parameters drive the separate {@link #shouldSendNewToken(Instant)} predicate
     * used by the token rotation flow.
     *
     ...
     *
     * @implNote
     * The clamp protects against AB-prop misconfigurations that would otherwise yield extreme bucket
     * sizes.
     *
     ...
     * {@code numBuckets} buckets of duration {@link #getTcTokenDuration(TcTokenMode)} is still valid.
     * The boundary instant is {@code (currentBucket - (numBuckets - 1)) * duration} where
    ...
    /**
     * Returns whether the given token timestamp falls before the role's expiry cutoff.
     *
     * <p>A token is expired when its issue timestamp is strictly less than the cutoff returned by
     * {@link #tokenExpirationCutoff(TcTokenMode)}.
     *
     ...
     * {@link ABProp#TCTOKEN_DURATION_SENDER}, uncapped unlike the receiver-side helper.
     *
     * @implNote
     ...
     *
     * @param tokenTimestamp the prior sender-token timestamp, or {@code null} when no token has been
     *                       issued
     ...
     *
     * <p>Used by callers that need to pass the token through a JSON or GraphQL surface rather than a
     * binary stanza child. The encoding is standard base64 with padding.
     *
     ...
     * {@code tctoken_num_buckets*} AB props, so the same token bytes have different validity windows
     * depending on whether the local device is the sender or the receiver of the token.
     */
