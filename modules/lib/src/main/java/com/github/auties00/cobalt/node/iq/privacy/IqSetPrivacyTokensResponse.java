 ...
 * {@link ServerError}) to surface either a token-issued acknowledgement or the error envelope.
 * The reply carries no per-token payload; the success signal is the envelope itself.
 *
 * @implNote
 * This implementation collapses WA Web's parse-and-throw flow ({@code setPrivacyTokensParser}
 * plus the {@code ServerStatusCodeError} thrown by
 * {@code WAWebSetPrivacyTokensJob.issuePrivacyToken} on a {@code 4xx}/{@code 5xx} reply) into a
...
public sealed interface IqSetPrivacyTokensResponse extends IqOperation.Response
    ...
    /**
     * Tries each {@link IqSetPrivacyTokensResponse} variant in priority order and returns the
     * first that parses cleanly.
     *
     * @apiNote
     * The dispatcher calls this immediately after receiving an inbound {@code <iq>} stanza whose
     * id matches an outstanding {@link IqSetPrivacyTokensRequest}; the empty {@link Optional}
     ...
     *
     * @apiNote
     * Carries no payload beyond the envelope echo; the only information conveyed is that the
     * batch was accepted. WA Web's parser reads the {@code id} attribute and returns
     * {@code {stanzaId}}; Cobalt's caller obtains the same id from the dispatcher and does not
     * need to surface it on the typed value.
     */
    ...
    final class Success implements IqSetPrivacyTokensResponse {
         ...
         * {@link SmaxIqResultResponseMixin#validate(Node, Node)}; no children are read, matching
         * WA Web's {@code setPrivacyTokensParser} which only echoes the stanza id.
         *
         ...
         *
         * @implNote
         * This implementation treats the variant as a singleton value type; all instances are
         * equal.
         */
        @Override
         ...
         *
         * @implNote
         * This implementation returns a class-stable hash consistent with
         * {@link #equals(Object)}.
         */
        @Override
        public int hashCode() {
         ...
         *
         * @implNote
         * This implementation emits a parameterless debug representation; the format is not
     ...
     * {@code WAWebSetPrivacyTokensJob.issuePrivacyToken} throws a {@code ServerStatusCodeError}
