...
/**
 * Parses the WhatsApp Web GraphQL response of the delete-business-AI-rule mutation built by
 * {@link BizAiRuleDeleteWhatsAppGraphQlRequest} into a {@link BusinessAiMutationResult}.
 *
 * <p>Projects the linked {@code xfb_meta_ai_biz_agent_wa_delete_rule} field, whose single
 * {@code success} scalar reports whether the rule was deleted, onto the shared mutation-result shape.
 * WhatsApp Web treats the mutation as successful only when {@code success} is exactly {@code true}.
 *
 * @see BizAiRuleDeleteWhatsAppGraphQlRequest
 */
@WhatsAppWebModule(moduleName = "WAWebBizAiRuleDeleteMutation")
public final class BizAiRuleDeleteWhatsAppGraphQlResponse implements WhatsAppGraphQlOperation.Response {
    /**
     ...
     */
    private BizAiRuleDeleteWhatsAppGraphQlResponse(BusinessAiMutationResult result) {
        this.result = result;
    }
     ...
     */
    public static Optional<BizAiRuleDeleteWhatsAppGraphQlResponse> of(JSONObject data) {
        if (data == null) {
            return Optional.empty();
        }

        var node = data.getJSONObject("xfb_meta_ai_biz_agent_wa_delete_rule");
        if (node == null) {
            return Optional.of(new BizAiRuleDeleteWhatsAppGraphQlResponse(null));
        }

        var result = new BusinessAiMutationResultBuilder()
                .success(Boolean.TRUE.equals(node.getBoolean("success")))
                .build();
        return Optional.of(new BizAiRuleDeleteWhatsAppGraphQlResponse(result));
    }
