...
final class SrtpRtpContext {
     ...
     */
    SrtpRtpContext(SrtpDirection direction, int ssrc, boolean sender, int authTagLen) {
    ...
    synchronized byte[] protect(byte[] rtp) {
        ...
        var out = new byte[rtp.length + authTagLen];
    ...
    synchronized byte[] unprotect(byte[] srtp) {
        ...
        if (srtp.length < RTP_FIXED_HEADER_LEN + authTagLen) {
            throw new WhatsAppCallException.Srtp("SRTP packet too short");
        }
        var dataLen = srtp.length - authTagLen;
        ...
        if (!SrtpDirection.constantTimeEquals(expected, 0, srtp, dataLen, authTagLen)) {
     ...
     * ROC in big-endian order, and the leading {@link #authTagLen} bytes of the output are
     * returned as the tag.
     *
     ...
     * {@code packet[0..dataLen)} into {@code packet[dataLen..dataLen + authTagLen)}.
     *
    ...
    private void appendAuthTag(byte[] packet, int dataLen, int roc) {
        ...
        System.arraycopy(tag, 0, packet, dataLen, authTagLen);
    }
