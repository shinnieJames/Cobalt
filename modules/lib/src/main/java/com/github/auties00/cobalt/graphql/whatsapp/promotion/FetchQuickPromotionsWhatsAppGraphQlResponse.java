...
/**
 * Parses the WhatsApp Web GraphQL response of the WhatsApp Business fetch-quick-promotions query built by
 * {@link FetchQuickPromotionsWhatsAppGraphQlRequest} into a list of {@link QuickPromotionSurfaceBatch}.
 *
 * <p>Reads the plural root {@code quick_promotion_batch_fetch_root} and projects each entry onto
 * a {@link QuickPromotionSurfaceBatch}: the surface identifier and the ordered list of eligible
 * {@link QuickPromotion banners}, each with their validity window, rendering
 * {@linkplain QuickPromotionCreative creatives}, and opaque logging blob.
 *
 * @see FetchQuickPromotionsWhatsAppGraphQlRequest
 */
@WhatsAppWebModule(moduleName = "WAWebFetchQuickPromotionsQuery")
public final class FetchQuickPromotionsWhatsAppGraphQlResponse implements WhatsAppGraphQlOperation.Response {
    /**
     ...
     */
    private FetchQuickPromotionsWhatsAppGraphQlResponse(List<QuickPromotionSurfaceBatch> batches) {
     ...
     *
     * <p>Reads the plural root {@code quick_promotion_batch_fetch_root} and projects each entry
     * onto a {@link QuickPromotionSurfaceBatch}; the returned {@link Optional} is empty when
     ...
     */
    public static Optional<FetchQuickPromotionsWhatsAppGraphQlResponse> of(JSONObject data) {
        if (data == null) {
            return Optional.empty();
        }

        var batches = parseBatches(data.getJSONArray("quick_promotion_batch_fetch_root"));
        return Optional.of(new FetchQuickPromotionsWhatsAppGraphQlResponse(batches));
    }

    /**
     * Projects the {@code quick_promotion_batch_fetch_root} array onto a list of
