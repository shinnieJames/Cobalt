 ...
 * collection identity, its products, and its review verdict onto the domain model, and folding the
 * sibling {@code paging.after} cursor into the collection's next-page cursor.
 *
 * @see QueryProductSingleCollectionWhatsAppGraphQlRequest
 */
@WhatsAppWebModule(moduleName = "WAWebQueryProductSingleCollectionQuery")
public final class QueryProductSingleCollectionWhatsAppGraphQlResponse implements WhatsAppGraphQlOperation.Response {
    /**
     ...
     * {@link BusinessProductCollection} whose {@link BusinessProductCollection#afterCursor()} carries
     * the {@code paging.after} cursor.
     *
    ...
    public static Optional<QueryProductSingleCollectionWhatsAppGraphQlResponse> of(JSONObject data) {
        if (data == null) {
            return Optional.empty();
        }
        var root = data.getJSONObject("xwa_product_catalog_get_single_collection");
        if (root == null) {
            return Optional.empty();
        }
        ...
        var collection = new BusinessProductCollectionBuilder()
                ...
                .afterCursor(after)
                .build();
        return Optional.of(new QueryProductSingleCollectionWhatsAppGraphQlResponse(collection));
    }
