 ...
 *
 * <p>Inbound data is read into a fixed-size read buffer, then decoded
 * as WebSocket frames.  Data frame payloads are fed into the next layer
 * (typically the {@link ApplicationLayerContext} for datagram reassembly).
 * Control frames (PING, PONG, CLOSE) are handled automatically.
 *
 * <p>This context sits between the TLS layer (or transport layer if
 * no TLS) and the application layer in the read pipeline.
 */
