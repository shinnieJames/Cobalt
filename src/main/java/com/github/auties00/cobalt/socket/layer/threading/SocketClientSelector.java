...
import com.github.auties00.cobalt.socket.layer.security.tls.TlsLayerContext;
...
public final class SocketClientSelector implements Runnable {
    ...
    public boolean addRead(SocketChannel channel, SocketClientTransportLayerContext.PendingRead read) {
        ...
        var ctx = (SocketClientContext) key.attachment();
        Optional<TunnelLayerContext> tunnelCtx = ctx.getLayerContext(SocketClientTunnelLayer.class);
    ...
    public void startTlsHandshake(SocketChannel channel, SocketClientLayerContext tlsContext, long timeout) throws IOException {
        ...
        var ctx = (SocketClientContext) key.attachment();
        if (!(tlsContext instanceof TlsLayerContext tls)) {
