 ...
 */
public final class HttpTunnelledTcpSocketClientTransport extends TcpSocketClientTransport {
    ...
    HttpTunnelledTcpSocketClientTransport(WhatsAppClientProxy.Http proxy) {
        this.proxy = proxy;
    }
    ...
    public TcpSocketClientContext connect(InetSocketAddress endpoint, TcpSocketClientListener listener) throws IOException, InterruptedException {
        ...
        for (var redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
            ...
            var ctx = super.connect(new InetSocketAddress(currentProxy.host(), currentProxy.port()), listener);
     ...
     */
    private void initProxyTls(TcpSocketClientContext ctx, String proxyHost, int proxyPort) throws IOException {
        try {
            ...
            ctx.initSsl(engine);
            startTlsHandshake(TLS_HANDSHAKE_TIMEOUT);
    ...
    private WhatsAppClientProxy.Http authenticate(WhatsAppClientProxy.Http currentProxy, long deadline) throws IOException {
        ...
        if (responseBuf != null && responseBuf.hasRemaining()) {
            if (!preSeedDatagram(responseBuf)) {
    ...
    private byte nextByte(long deadline) throws IOException {
        ...
        var length = readBinary(responseBuf, false);
    ...
    private void sendConnectRequest(String authorizationHeader) {
        ...
        builder.append("\r\n");
        sendBinary(ByteBuffer.wrap(builder.toString().getBytes(StandardCharsets.US_ASCII)));
    }
