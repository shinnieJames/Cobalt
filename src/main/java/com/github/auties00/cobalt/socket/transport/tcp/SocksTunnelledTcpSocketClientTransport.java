 ...
 */
public final class SocksTunnelledTcpSocketClientTransport extends TcpSocketClientTransport {
     ...
     */
    SocksTunnelledTcpSocketClientTransport(WhatsAppClientProxy.Socks proxy) {
        this.proxy = proxy;
    }
    ...
    public TcpSocketClientContext connect(InetSocketAddress endpoint, TcpSocketClientListener listener) throws IOException, InterruptedException {
        ...
        var ctx = super.connect(new InetSocketAddress(proxyHost, proxyPort), listener);
    ...
    private void performSocks5Handshake(WhatsAppClientProxy.Socks.V5 socks) throws IOException {
        ...
        sendBinary(ByteBuffer.wrap(greeting));

        var serverChoice = ByteBuffer.allocate(2);
        readBinary(serverChoice, true);
        ...
        if (chosenMethod != METHOD_NO_AUTH) {
            ...
            switch (authenticator) {
                case WhatsAppClientProxyAuthenticator.Socks.V5.UserPassword credentials -> {
                    ...
                    authRequest.flip();
                    sendBinary(authRequest);

                    var authResponse = ByteBuffer.allocate(2);
                    readBinary(authResponse, true);
    ...
    private void handleSocks5Reply() throws IOException {
        ...
        readBinary(ByteBuffer.allocate(remainingBytes), true);
    }
